...
# Class to manage loading data for bayesian algorithms
class BayesianInputData:
	...
	def __LoadGem(self, config, chrom):
		...
		if os.path.exists(path_to_gem_file):
		    ...
		    path_to_gem_file=path_to_gem_file.replace(".gem", "_gem.txt")
	...
	def __LoadLambda(self, config, stats):
		...
		# For each percent store corresponding lambda
		# This is used in bayesian for access optimisation
	...
	# Calculate lambda and cache it in lambda file
	def __CalculateLambda(self, config, stats):
		...
		lambda_norm.sort(key = lambda l : l[0])
		lambda_abnorm = filter(partial(is_not, None), lambda_abnorm)
		lambda_abnorm.sort(key = lambda l : l[0])
